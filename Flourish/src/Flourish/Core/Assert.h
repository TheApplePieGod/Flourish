...
#define FL_INTERNAL_ASSERT_NOMSG(log, task, check) FL_INTERNAL_ASSERT_IMPL(log, task, check, "Assertion '%s' failed at %s:%d", check, std::filesystem::path(__FILE__).filename().string().c_str(), __LINE__)

#define FL_INTERNAL_ASSERT_FIND(_1, _2, _3, _4, _5, macro, ...) macro
#define FL_INTERNAL_ASSERT_PICK(...) FL_EXPAND_ARGS( FL_INTERNAL_ASSERT_FIND(__VA_ARGS__, FL_INTERNAL_ASSERT_MSGARGS, FL_INTERNAL_ASSERT_MSGARGS, FL_INTERNAL_ASSERT_MSGARGS, FL_INTERNAL_ASSERT_MSG, FL_INTERNAL_ASSERT_NOMSG) )
